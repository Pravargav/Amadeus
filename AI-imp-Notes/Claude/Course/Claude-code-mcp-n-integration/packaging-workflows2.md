...
## Certification Quick Revision

### Cost

- Skills add context cost when activated.
- Plugins add installation and maintenance cost.
- Plugins reduce repeated manual setup for teams.

### Complexity

- Installation and execution are different.
- A plugin can install successfully and still fail when used.
- Absolute paths are a common failure source.

### Risk

- Hooks and deny rules are not included automatically.
- Required guardrails must be bundled explicitly.
...
- Never assume every developer has the same shell configuration.
