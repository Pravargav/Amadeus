-> FX rates - foreign exchange rates - we import from bw to sac

-> EBIT - earnings before interest and taxes

-> Transaction data directly posted to sac

-> Master data is fetched from BW

-> Cash Flow Reporting model import data from APH(amadues planning hub) - FFC(financial forecast)

-> Cash Flow Reporting model also takes some data from planning model and account from bw system

-> The planning model imoprt master data from bw and transactional data are entered by users who want to plan stories.

-> Any types of currencies converted to either EUR or USD .So derived has two groups(clusters) EUR or USD.(direct has 198 currencies but derived has 2 currencies only).

-> AAM - analytics access management
